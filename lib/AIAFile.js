    ...
    */
    ...
    _loadContent(req, zip, fileInfo) {
        if (fileInfo.name.endsWith('project.properties')) {
            ...
            if (!fileInfo._data.compressedSize) {
                req.logger.info('Empty resource  detected. Substituting a stubbed content', {resource: fileInfo.name})
                zip.file(fileInfo.name, 'Empty resource not supported. This is stub that can be replaced.')
            }
